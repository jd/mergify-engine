...
def test_get_pull_request_rule():
    ...
    pull_request._get_combined_status = mock.Mock()
    fake_ci = mock.Mock()
    fake_ci.context = "continuous-integration/fake-ci"
    fake_ci.state = "success"
    status = mock.Mock()
    status.statuses = [fake_ci]
    pull_request._get_combined_status.return_value = status
    ...
    pull_request_rules = rules.PullRequestRules([r1])
